...
import static com.hongyu.util.Constants.Xuqian;
...
public class XuqianListener implements TaskListener {
		...
		for(HyAdmin admin : admins) {
			...
			if (role.getHyRoleAuthorities().size() > 0) {
				for (HyRoleAuthority hyRoleAuthority : role.getHyRoleAuthorities()) {
					if (Xuqian.equals(hyRoleAuthority.getAuthoritys().getRequestUrl())) {
