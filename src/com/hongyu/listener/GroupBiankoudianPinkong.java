...
import static com.hongyu.util.Constants.GroupBiankoudian;
...
public class GroupBiankoudianPinkong implements TaskListener {
		...
		for(HyAdmin admin : admins) {
			...
			if (role.getHyRoleAuthorities().size() > 0) {
				for (HyRoleAuthority hyRoleAuthority : role.getHyRoleAuthorities()) {
					if (GroupBiankoudian.equals(hyRoleAuthority.getAuthoritys().getRequestUrl())) {
