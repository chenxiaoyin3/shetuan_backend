...
import static com.hongyu.util.Constants.hotelandsceneroomprice;
...
public class HotelandsceneRoomPriceListener implements TaskListener {
	...
	public void notify(DelegateTask delegateTask) {
		...
		Set<HyAdmin> admins = new HashSet<HyAdmin>();
		for(Department d : ds) {
		...
		for(HyAdmin admin : admins) {
			...
			if (role.getHyRoleAuthorities().size() > 0) {
				for (HyRoleAuthority hyRoleAuthority : role.getHyRoleAuthorities()) {
					if (hotelandsceneroomprice.equals(hyRoleAuthority.getAuthoritys().getRequestUrl())) {
