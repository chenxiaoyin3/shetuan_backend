 ...
 * 分公司充值审核 - 指定财务审核人
 */
public class AccountantReview_BranchRecharge_Listener implements TaskListener {
    ...
    public void notify(DelegateTask delegateTask) {
        ...
        for (HyAdmin admin : admins) {
            ...
            if (role.getHyRoleAuthorities().size() > 0) {
                for (HyRoleAuthority hyRoleAuthority : role.getHyRoleAuthorities()) {
                    if ("admin/branch/review".equals(hyRoleAuthority.getAuthoritys().getRequestUrl())) {
        ...
        /****调用微信接口 发送微信推送*****/
        String message = "您有新工作需要审核，请尽快完成。";
        SendMessageQyWx.sendWxMessage(QyWxConstants.ZONG_BU_CAI_WU_QYWX_APP_AGENT_ID, users, null, message);
    }
}
