...
public class StoreShouHouFinanceListener implements TaskListener{
		...
		for(HyAdmin admin:admins){
			...
			if(role.getHyRoleAuthorities().size()>0){
				for(HyRoleAuthority hyRoleAuthority:role.getHyRoleAuthorities()){
					if(Constants.CaiwuAuditStoreShouHou.equals(hyRoleAuthority.getAuthoritys().getRequestUrl())){
