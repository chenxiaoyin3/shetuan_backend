...
public class AccountantReview_SupplierDismissOrder_Listener implements TaskListener {
		...
		for(HyAdmin admin : admins) {
			...
			if (role.getHyRoleAuthorities().size() > 0) {
				for (HyRoleAuthority hyRoleAuthority : role.getHyRoleAuthorities()) {
					if ("admin/accountant/dissmissOrder".equals(hyRoleAuthority.getAuthoritys().getRequestUrl())) {
