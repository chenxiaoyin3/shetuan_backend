...
public class AccountantReview_StoreDeposit_Listener implements TaskListener {
	...
	public void notify(DelegateTask delegateTask) {
		...
		if(storeType == 0){
			...
			for(HyAdmin admin : admins) {
				...
				if (role.getHyRoleAuthorities().size() > 0) {
					for (HyRoleAuthority hyRoleAuthority : role.getHyRoleAuthorities()) {
						if ("admin/accountant/storeDeposit".equals(hyRoleAuthority.getAuthoritys().getRequestUrl())) {
			...
			if (requestAttributes != null) {
                String username  = (String) runtimeService.getVariable(delegateTask.getExecutionId(), "inputUser");
