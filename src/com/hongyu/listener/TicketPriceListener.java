...
import static com.hongyu.util.Constants.sceneticketprice;
...
public class TicketPriceListener implements TaskListener {
	@Override
		...
		for(HyAdmin admin : admins) {
			...
			if (role.getHyRoleAuthorities().size() > 0) {
				for (HyRoleAuthority hyRoleAuthority : role.getHyRoleAuthorities()) {
					if (sceneticketprice.equals(hyRoleAuthority.getAuthoritys().getRequestUrl())) {
