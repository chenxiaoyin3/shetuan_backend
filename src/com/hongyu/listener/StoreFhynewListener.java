...
import static com.hongyu.util.Constants.storeFhynew;
...
/*新建非虹宇门店审核 - 指定分公司副总 */
public class StoreFhynewListener implements TaskListener {
	@Override
	public void notify(DelegateTask delegateTask) {
		...
		HyDepartmentModel model = hyDepartmentModelService.find("分公司");
		...
		for(HyAdmin admin : admins) {
			...
			if (role.getHyRoleAuthorities().size() > 0) {
				for (HyRoleAuthority hyRoleAuthority : role.getHyRoleAuthorities()) {
					if (storeFhynew.equals(hyRoleAuthority.getAuthoritys().getRequestUrl())) {
