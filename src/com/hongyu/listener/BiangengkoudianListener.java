...
import static com.hongyu.util.Constants.Biangengkoudian;
...
public class BiangengkoudianListener implements TaskListener {
		...
		for(HyAdmin admin : admins) {
			...
			if (role.getHyRoleAuthorities().size() > 0) {
				for (HyRoleAuthority hyRoleAuthority : role.getHyRoleAuthorities()) {
					if (Biangengkoudian.equals(hyRoleAuthority.getAuthoritys().getRequestUrl())) {
