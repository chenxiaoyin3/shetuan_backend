...
import static com.hongyu.util.Constants.subscribePrice;
...
public class SubscribeTicketListener implements TaskListener {
			...
			for (HyAdmin admin : admins) {
				...
				if (role.getHyRoleAuthorities().size() > 0) {
					for (HyRoleAuthority hyRoleAuthority : role.getHyRoleAuthorities()) {
						if (subscribePrice.equals(hyRoleAuthority.getAuthoritys().getRequestUrl())) {
