...
public class AccountantReview_DistributorPreCharge_Listener implements TaskListener {
		...
		for(HyAdmin admin : admins) {
			...
			if (role.getHyRoleAuthorities().size() > 0) {
				for (HyRoleAuthority hyRoleAuthority : role.getHyRoleAuthorities()) {
					if ("admin/accountant/distributorPrecharge".equals(hyRoleAuthority.getAuthoritys().getRequestUrl())) {
