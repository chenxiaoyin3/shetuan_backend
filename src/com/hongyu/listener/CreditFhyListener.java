...
import static com.hongyu.util.Constants.creditFhy;
...
public class CreditFhyListener implements TaskListener {
	@Override
		...
		for(HyAdmin admin : admins) {
			...
			if (role.getHyRoleAuthorities().size() > 0) {
				for (HyRoleAuthority hyRoleAuthority : role.getHyRoleAuthorities()) {
					if (creditFhy.equals(hyRoleAuthority.getAuthoritys().getRequestUrl())) {
