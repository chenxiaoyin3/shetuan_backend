...
import static com.hongyu.util.Constants.visaPrice;
...
public class VisaPriceListener implements TaskListener {
	@Override
		...
		for(HyAdmin admin : admins) {
			...
			if (role.getHyRoleAuthorities().size() > 0) {
				for (HyRoleAuthority hyRoleAuthority : role.getHyRoleAuthorities()) {
					if (visaPrice.equals(hyRoleAuthority.getAuthoritys().getRequestUrl())) {
