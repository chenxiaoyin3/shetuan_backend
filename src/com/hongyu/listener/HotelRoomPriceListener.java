...
import static com.hongyu.util.Constants.hotelroomprice;
...
public class HotelRoomPriceListener implements TaskListener {
	@Override
		...
		for(HyAdmin admin : admins) {
			...
			if (role.getHyRoleAuthorities().size() > 0) {
				for (HyRoleAuthority hyRoleAuthority : role.getHyRoleAuthorities()) {
					if (hotelroomprice.equals(hyRoleAuthority.getAuthoritys().getRequestUrl())) {
