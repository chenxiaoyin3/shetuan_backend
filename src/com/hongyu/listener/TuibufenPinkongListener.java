...
import static com.hongyu.util.Constants.Tuibufen;
...
public class TuibufenPinkongListener implements TaskListener {
		...
		for(HyAdmin admin : admins) {
			...
			if (role.getHyRoleAuthorities().size() > 0) {
				for (HyRoleAuthority hyRoleAuthority : role.getHyRoleAuthorities()) {
					if (Tuibufen.equals(hyRoleAuthority.getAuthoritys().getRequestUrl())) {
