...
/** 向酒店/门票/酒加景付款审核 - 指定财务审核人 - 总公司 */
@SuppressWarnings("serial")
public class AccountantReview_TicketPay_Listener implements TaskListener {
		...
		for (HyAdmin admin : admins) {
			...
			if (role.getHyRoleAuthorities().size() > 0) {
				for (HyRoleAuthority hyRoleAuthority : role.getHyRoleAuthorities()) {
					if ("admin/ticketPay".equals(hyRoleAuthority.getAuthoritys().getRequestUrl())) {
