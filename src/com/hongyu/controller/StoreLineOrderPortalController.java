...
@Controller
@RequestMapping("/admin/storeLineOrderPortal/")
public class StoreLineOrderPortalController {
...
//			filters.add(Filter.in("operator", hyAdmins));
			...
			if(source==null){
				filters.add(Filter.gt("source",0));
			}else{
			...
			for (HyOrder tmp : page.getRows()) {
				...
				if(tmp.getXianlutype()!=null){
					if(tmp.getXianlutype()>3){
						...
						if(tmp.getTianshu()!=null && tmp.getTianshu()>1){
							contractType = 1;
						}else{
							contractType = 0;
						}
				...
				map.put("createtime", tmp.getCreatetime());
				if (operator!=null && operator.equals(admin)) {
					if (co == CheckedOperation.view) {
						map.put("privilege", "view");
					} else {
						map.put("privilege", "edit");
				...
				map.put("supplierName", tmp.getSupplier()!=null?tmp.getSupplier().getName():"");
				map.put("fatuandate", tmp.getFatuandate());
				map.put("discountedPrice", tmp.getDiscountedPrice());
