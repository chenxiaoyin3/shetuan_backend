 ...
 */
@RestController
@RequestMapping("/admin/pinkong/hotelandsceneroomprice")
@Transactional(propagation = Propagation.REQUIRED)
public class Review_ticket_hotelandscenepPriceController {
	@Resource
	...
	@ResponseBody
	public Json listview(HyTicketHotelandscene queryParam,String submitName,Integer state,Pageable pageable,HttpSession session)
	{
		Json json=new Json();
		try{
			String username = (String) session.getAttribute(CommonAttributes.Principal);
			List<Filter> filters = FilterUtil.getInstance().getFilter(queryParam);
			List<HyTicketHotelandscene> hyTicketHotelandscenes=hyTicketHotelandsceneService.findList(null,filters,null);
			if(hyTicketHotelandscenes.isEmpty()) {
				json.setMsg("查询成功");
			    json.setSuccess(true);
			    json.setObj(new Page<HyTicketHotelandscene>());
			}
			else {
				List<Filter> roomFilter=new ArrayList<Filter>();
				roomFilter.add(Filter.in("hyTicketHotelandscene", hyTicketHotelandscenes));
				if(submitName!=null&&!submitName.equals(""))
				{
					...
					if(adminList.size()==0){
						...
						roomFilter.add(Filter.in("submitter",adminList));
						List<HyTicketHotelandsceneRoom> hotelandsceneRooms=hyTicketHotelandsceneRoomService.findList(null,roomFilter,null);
						if(hotelandsceneRooms.size()==0) {
							json.setMsg("查询成功");
						    json.setSuccess(true);
						    json.setObj(new Page<HyTicketHotelandsceneRoom>());
						}
						else {
...
//								List<Task> tasks = taskService.createTaskQuery().taskCandidateUser(username).list();
							    List<Task> tasks = ActivitiUtils.getTaskList(username, "hotelandsceneRoomPriceProcess");
								for(Task task:tasks) {
							    	String processInstanceId = task.getProcessInstanceId();
							    	for(HyTicketHotelandsceneRoom tmp:hotelandsceneRooms) {
							    		if(processInstanceId.equals(tmp.getProcessInstanceId())) {
							    			HashMap<String, Object> map = new HashMap<>();
							    			map.put("roomId", tmp.getId());
							    			map.put("state", 0);
							    			map.put("productId", tmp.getHyTicketHotelandscene().getProductId());
							    			map.put("productName", tmp.getHyTicketHotelandscene().getProductName());
											...
											map.put("roomType",tmp.getRoomType());
											map.put("isWifi",tmp.getIsWifi());
											map.put("isWindow",tmp.getIsWindow());
											map.put("isBathroom",tmp.getIsBathroom());
											map.put("available", tmp.getAvailable());
											map.put("breakfast", tmp.getBreakfast());
											ans.add(map);
							    		}
							    	}
							    }
...
//										.finished().taskAssignee(username).list();
							    List<HistoricTaskInstance> historicTaskInstances = ActivitiUtils.getHistoryTaskList(username, "hotelandsceneRoomPriceProcess");
...
//									.finished().taskAssignee(username).list();
	...
	@ResponseBody
	public Json add(Long roomId)
	{
		Json json=new Json();
		try {
			HyTicketHotelandsceneRoom room=hyTicketHotelandsceneRoomService.find(roomId);
			HyTicketHotelandscene hotelandscene=room.getHyTicketHotelandscene();
			...
			map.put("refundType", hotelandscene.getRefundType());
			...
			filters.add(Filter.eq("productId",hotelandscene.getId()));
