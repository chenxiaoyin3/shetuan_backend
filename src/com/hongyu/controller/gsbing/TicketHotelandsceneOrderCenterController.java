 ...
 */
@Controller
@RequestMapping("admin/ticket_order_center/hotelandscene")
public class TicketHotelandsceneOrderCenterController {
	@Resource
	...
	@ResponseBody
	public Json listview(Pageable pageable, HyOrder queryParam,Integer star,@DateTimeFormat(pattern="yyyy-MM-dd") Date startTime, 
			@DateTimeFormat(pattern="yyyy-MM-dd") Date endTime,String customerName,HttpSession session, HttpServletRequest request)
	{
		Json json=new Json();
		try {
			/**
...
//			CheckedOperation co = (CheckedOperation) request.getAttribute("co");
			...
			if(star!=null) {
				for(HyOrder hyOrder:page.getRows()) {
					filters.add(Filter.eq("order", hyOrder));
					filters.add(Filter.eq("type", 5)); //5-酒加景
					List<HyOrderItem> ordetItems=hyOrderItemService.findList(null,filters,null);
					filters.clear();
					HyOrderItem hyOrderItem=ordetItems.get(0);
					
					//added in 20190712,增加按游客姓名筛选的功能
					if(customerName!=null) {
						boolean hasName = false;
						for(HyOrderCustomer customer:hyOrderItem.getHyOrderCustomers()) {
					...
					if(hyTicketHotelandscene.getHotelStar()==star) {
	...
	public Json detail(Long id)
	{
		...
		try {
			...
			//官网选择门店
				...
				map.put("orderMoney", hyOrder.getWaimaiMoney());
			}
	...
	public Json productList(Long id)
	{
		...
		try {
			...
			for(HyOrderItem item:orderItems) {
				...
				//如果是门店订单
				if(hyOrder.getSource()==0) {
					map.put("productPrice", hyOrder.getJiusuanMoney());
				}
				else {
	...
	//供应商订单中心列表页
	@RequestMapping("supplier/list/view")
	@ResponseBody
	public Json supplierListview(Pageable pageable, HyOrder queryParam,@DateTimeFormat(pattern="yyyy-MM-dd") Date startTime, 
	...
	public Json storeRefundList(Long id) {
		...
		try {
			...
			for (HyOrderItem item : order.getOrderItems()) {
				...
				myOrderItem.setJiesuanRefund((item.getJiesuanPrice().subtract(order.getStoreFanLi()).subtract(order.getDiscountedPrice())
						.setScale(2, BigDecimal.ROUND_HALF_UP)).multiply(ticketRefundPercentage));
