 ...
 */
@RestController
@RequestMapping("/admin/gystuichu/shenhe/")
public class GystuichushenheController {
	...
	/**
	 * 审核(通过或者驳回)供应商退押金申请
	 * @param id 退押金申请id comment 驳回批注   shenheStatus 审核状态
	 * 		
	 * @return
	 */
	@Transactional
			...
			if (processInstanceId == null || processInstanceId == "") {
				...
				HashMap<String, Object> map = new HashMap<>();
					if(shenheStatus.equals("yitongguo")) {
						...
						if(task.getTaskDefinitionKey().equals("usertask3")) { //如果是品控审核需要单独处理，因为有三个流转的地方
							List<Filter> filters = new ArrayList<>();
							filters.add(Filter.eq("eduleixing", Eduleixing.tuiyajinfuzong));
						...
						} else if (task.getTaskDefinitionKey().equals("usertask5")) {
							...
							contract.setContractStatus(ContractStatus.tuichu);
							contract.setReturnDeposit(BigDecimal.ZERO);
							HyAdmin liable = contract.getLiable();
...
//							hySupplierContractService.update(contract);
							//将供应商的所有线路置为已下线
							List<Filter> filters = new ArrayList<>();
							filters.add(Filter.eq("contract", contract));
							List<HyLine> lines = hyLineService.findList(null, filters, null);
							for(HyLine temp : lines) {
								temp.setIsSale(IsSaleEnum.yixia);
								hyLineService.update(temp);
							}
						}
