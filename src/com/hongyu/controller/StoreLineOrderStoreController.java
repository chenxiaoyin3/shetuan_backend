...
@Controller
@RequestMapping("/admin/storeLineOrderStore/")
public class StoreLineOrderStoreController {
	...
	public Json list(Pageable pageable, @DateTimeFormat(pattern="yyyy-MM-dd") Date startTime, 
			...
			Integer refundstatus, Integer source, String orderNumber, String name,Integer status, String guestName) {
		Json json = new Json();
		try {
...
//			List<HashMap<String, Object>> result = new ArrayList<>();
			...
			if(startDate != null) {
				filters.add(Filter.ge("createtime", DateUtil.getStartOfDay(startDate)));
			}
			if(endDate != null) {
				filters.add(Filter.le("createtime", DateUtil.getEndOfDay(endDate)));
			}
...
//			List<HyOrderApplication> hyOrderApplicationList=hyOrderApplicationService.findList(id);
//			List<HyOrderApplicationItem> hyOrderApplicationItemList= new LinkedList<>();
//			if(hyOrderApplicationList!=null) {
//				for(HyOrderApplication tmp:hyOrderApplicationList) {
//					if(tmp.getHyOrderApplicationItems()!=null&&tmp.getHyOrderApplicationItems().size()>0) {
//						hyOrderApplicationItemList.addAll(tmp.getHyOrderApplicationItems());
//					}
//				}
//			}
			for (HyOrderItem tmp : hyOrder.getOrderItems()) {
			...
			List<Filter> filtersOne=new ArrayList<Filter>();
			HyOrderItem hyOrderItem=new HyOrderItem();
			List<HyOrderApplicationItem> hyOrderApplicationItemList=new ArrayList<>();
//			Iterator<HyOrderCustomer> iter=result.iterator();
//			while(iter.hasNext()) {
//				comMap.put("name", iter.next().getName());
//				resultList.add(comMap);
//			}
			HyGroup group  = hyGroupService.find(hyOrder.getGroupId());
			BigDecimal lineRefundPercentage = hyOrderService.getLineRefundPercentage(hyOrder).multiply(BigDecimal.valueOf(0.01));
			for(HyOrderCustomer temp:result) {
				HashMap<String,Object> comMap = new HashMap<String,Object>();
				comMap.put("name", temp.getName());
				comMap.put("type", temp.getType());
				comMap.put("gender", temp.getGender());
				comMap.put("birthday", temp.getBirthday());
				comMap.put("age", temp.getAge());
				comMap.put("phone", temp.getPhone());
				comMap.put("isCoupon", temp.getIsCoupon());
				comMap.put("isInsurance", temp.getIsInsurance());
				comMap.put("insuranceId", temp.getInsuranceId());
				comMap.put("salePrice", temp.getSalePrice());
				comMap.put("settlementPrice", temp.getSettlementPrice());
				comMap.put("certificateType", temp.getCertificateType());
				comMap.put("certificate", temp.getCertificate());
				comMap.put("certificateTime", temp.getCertificateTime());
				comMap.put("passportTime", temp.getPassportTime());
				comMap.put("country", temp.getCountry());
				comMap.put("surname", temp.getSurname());
				comMap.put("firstname", temp.getFirstname());
				filtersOne.add(Filter.eq("itemId", temp.getOrderItem().getId()));
				hyOrderApplicationItemList=hyOrderApplicationItemService.findList(null, filtersOne, null);
				hyOrderItem=temp.getOrderItem();
				if(hyOrderItem!=null) {
						HyOrderApplicationItem preRefundItem = hyOrderApplicationItemService.getTotalRefund(hyOrderApplicationItemService.getItemsByOrderItem(hyOrderItem.getId()));
						BigDecimal maxTuiKuanMoney=(hyOrderItem.getJiesuanPrice().subtract(group.getFanliMoney().multiply(
										BigDecimal.valueOf(hyOrderItem.getHyOrderCustomers().size())))).multiply
										(hyOrder.getJiesuanMoney1().subtract(hyOrder.getStoreFanLi()).subtract(hyOrder.getDiscountedPrice())).divide(
												hyOrder.getJiesuanMoney1().subtract(hyOrder.getStoreFanLi()),2,RoundingMode.HALF_UP
										)
								.multiply(lineRefundPercentage);
						if(!hyOrderApplicationItemList.isEmpty()) {
							for(HyOrderApplicationItem flag:hyOrderApplicationItemList) {
								Integer tempType=flag.getHyOrderApplication().getType();
								if(tempType.equals(0)&&(preRefundItem.getJiesuanRefund().compareTo(BigDecimal.ZERO)!=0)&&(preRefundItem.getJiesuanRefund().compareTo(maxTuiKuanMoney)<0)) {
									comMap.put("status", "部分金额退团");
								}
								else if(tempType.equals(0)&&(preRefundItem.getJiesuanRefund().compareTo(maxTuiKuanMoney)==0)) {
									comMap.put("status", "全部金额退团");
								}
								else if(tempType.equals(1)&&(preRefundItem.getJiesuanRefund().compareTo(BigDecimal.ZERO)!=0)&&(preRefundItem.getJiesuanRefund().compareTo(maxTuiKuanMoney)<0)) {
									comMap.put("status", "部分金额售后");
								}
								else if(tempType.equals(1)&&(preRefundItem.getJiesuanRefund().compareTo(maxTuiKuanMoney)==0)) {
									comMap.put("status", "全部金额售后");
								}
								else if(tempType.equals(2)) {
									comMap.put("status", "消团");
								}
								else{
									comMap.put("status", "正常");
								}
							}
						}
						else {
...
//			map.put("rows", result.subList((pg - 1) * rows, pg * rows > result.size() ? result.size() : pg * rows));
			...
			map.put("pageNumber", pg);
			map.put("pageSize", rows);
			map.put("rows", resultList.subList((pg - 1) * rows, pg * rows > resultList.size() ? resultList.size() : pg * rows));
			//end by cqx 2019-6-20
			json.setSuccess(true);
			json.setMsg("获取成功");
			json.setObj(map);
		} catch (Exception e) {
	...
	public Json scgList(Long id) {
		...
		try {
			...
			for (HyOrderItem item : order.getOrderItems()) {
				...
				HyOrderApplicationItem preRefundItem = hyOrderApplicationItemService.getTotalRefund(hyOrderApplicationItemService.getItemsByOrderItem(item.getId()));
				...
				if(item.getType()==8){	//线路其他价格
					...
					.multiply(lineRefundPercentage).subtract(preRefundItem.getJiesuanRefund()));
					myOrderItem.setWaimaiPrice(item.getWaimaiPrice());
					myOrderItem.setWaimaiRefund(item.getWaimaiPrice().multiply
							(order.getJiesuanMoney1().subtract(order.getStoreFanLi()).subtract(order.getDiscountedPrice())).divide(
									order.getJiesuanMoney1().subtract(order.getStoreFanLi()),2,RoundingMode.HALF_UP
							)
					.multiply(lineRefundPercentage).subtract(preRefundItem.getWaimaiRefund()));
				...
				myOrderItem.setBaoxianJiesuanRefund(myOrderItem.getBaoxianJiesuanPrice().subtract(preRefundItem.getBaoxianJiesuanRefund()));
				myOrderItem.setBaoxianWaimaiPrice(hyOrderItemService.getBaoxianWaimaiPrice(item));
				myOrderItem.setBaoxianWaimaiRefund(myOrderItem.getBaoxianWaimaiPrice().subtract(preRefundItem.getBaoxianWaimaiRefund()));

				if(myOrderItem.getJiesuanRefund().equals(BigDecimal.ZERO) || myOrderItem.getWaimaiRefund().equals(BigDecimal.ZERO)){
					myOrderItem.setReturnNumber(myOrderItem.getNumber());
				}
	...
	@ResponseBody
	public Json storeCancelGroupApply(@RequestBody HyOrderApplication application,Boolean isOnlyBaoxian, HttpSession session) {
		/**
		 * 获取当前用户
		 */
		String username = (String) session.getAttribute(CommonAttributes.Principal);
		...
		try {
			...
			if(isOnlyBaoxian==null)isOnlyBaoxian=false;
			if(isOnlyBaoxian){
				application.setJiesuanMoney(BigDecimal.ZERO);
				application.setWaimaiMoney(BigDecimal.ZERO);
				for (HyOrderApplicationItem item : application.getHyOrderApplicationItems()) {
					item.setJiesuanRefund(BigDecimal.ZERO);
					item.setWaimaiRefund(BigDecimal.ZERO);
				}
			}
			...
			if(isOnlyBaoxian){
				...
				Authentication.setAuthenticatedUserId(provider.getUsername());
				taskService.addComment(task.getId(), pi.getProcessInstanceId(),
						("审核通过") + ":" + 1);
...
//					}
	...
	@ResponseBody
	public Json storeCustomerServiceApply(@RequestBody HyOrderApplication application,Boolean isOnlyBaoxian, HttpSession session) {
		/**
		 * 获取当前用户
		 */
		String username = (String) session.getAttribute(CommonAttributes.Principal);
