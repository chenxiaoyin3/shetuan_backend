...
@RequestMapping("/admin/secondticket/secondaudit/")
public class TicketAuditHaveToChangeTwo {
			...
			try {
				...
				if (auditStatus.equals(1)) { // 如果审核通过
					...
					if (task.getTaskDefinitionKey().equals("usertask2")) { // 如果供应商
						...
						//2019-6-10 做一个容错
						BigDecimal rongcuo = application.getBaoxianJiesuanMoney();
						BigDecimal tuiKuan = new BigDecimal(0);
						if(rongcuo != null){
							tuiKuan = application.getJiesuanMoney().add(application.getBaoxianJiesuanMoney());
						} else {
							tuiKuan = application.getJiesuanMoney();
						}
					...
					// add by wj 2019/7/20  门票退款驳回短信提示
