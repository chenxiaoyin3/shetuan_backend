...
@Controller
@RequestMapping("/admin/business/weBusinessNonHyManagement/")
@Transactional(propagation=Propagation.REQUIRED)
public class WeBusinessNonHyManagementController {
	...
	public Json list(Pageable pageable,WeBusiness weBusiness,HttpSession session,HttpServletRequest request){
		...
		try {
			...
			filters.add(filter);
			Filter filter2=Filter.eq("type", WeBusiness.nonhytype);
			filters.add(filter2);
...
//					storeIds.add((Long)tmp.getId());
//				}
//				filters.add(Filter.in("storeId", storeIds));
//			}
