...
@Controller
@RequestMapping("admin/ticket_hotel_order_scs/")
public class HyTicketHotelOrderScsController {
...
//			}
	...
	public Json scsList(Long id) {
		...
		try {
			...
			for (HyOrderItem item : order.getOrderItems()) {
				// 优惠金额
				BigDecimal discount = order.getDiscountedPrice().multiply(
						(
								item.getJiesuanPrice()
						).divide(
								order.getJiesuanMoney1(),2, RoundingMode.HALF_UP
						)
				).multiply(BigDecimal.valueOf((item.getNumber()-item.getNumberOfReturn())/(item.getNumber())));
				...
				myOrderItem.setReturnNumber(item.getNumberOfReturn());
				myOrderItem.setNumber(item.getNumber());
				myOrderItem.setJiesuanPrice(item.getJiesuanPrice().subtract(discount).subtract(group.getFanliMoney().multiply(
						BigDecimal.valueOf(item.getNumber()-item.getNumberOfReturn()))));
				myOrderItem.setJiesuanRefund(BigDecimal.ZERO);
				myOrderItem.setWaimaiPrice(item.getWaimaiPrice().subtract(discount).subtract(group.getFanliMoney().multiply(
						BigDecimal.valueOf(item.getNumber()-item.getNumberOfReturn()))));
	...
	public Json storeCustomerServiceAudit(Long id, String comment, Integer auditStatus, HttpSession session) {
		...
		try {
			...
			if (auditStatus.equals(1)) { // 如果审核通过
				...
				// add by wj 2019/7/20  酒店售后驳回短信提示
