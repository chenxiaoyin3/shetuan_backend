...
@Controller
@RequestMapping("admin/ticket_hotel_order_store/")
public class HyTicketHotelOrderStoreController {
	...
	public Json pageView(Pageable pageable,Integer payStatus,Integer refundStatus,Integer star,
			String hotelName,String orderNumber,String startDate,
			String endDate, String guestName, HttpSession session,HttpServletRequest request){
		Json json = new Json();
		try {
			...
			if (startDate != null && !startDate.equals("")) {
				Date sTime = sdf.parse(startDate.substring(0, 10) + " " + "00:00:00");
				sb.append("  AND o1.createtime >=  " + "'" + sdf.format(sTime) + "'");
			}
			if (endDate != null && !endDate.equals("")) {
				Date eTime = sdf.parse(endDate.substring(0, 10) + " " + "23:59:59");
				sb.append("  AND o1.createtime <=  " + "'" + sdf.format(eTime) + "'");
			}
			...
			if(guestName!=null){
				sb.append(" and o1.contact like '%" + guestName +"%'");
			}
...
//			}
	...
	public Json detailView(Long id,HttpSession session) {
		...
		try {
			...
			map.put("productStatus", (hyOrder.getGuideCheckStatus()==1 && (new Date()).compareTo(hyOrderItem.getStartDate())>=0)?true:false);
	...
	public Json itemDetailView(Long id,HttpSession session) {
		...
		try {
			...
			for(HyOrderItem hyOrderItem:hyOrderItems) {
				...
				map.put("productId", hyTicketHotelRoom.getProductId());
				...
				map.put("startDate",hyOrderItem.getStartDate());
                map.put("waimaiPrice", hyOrderItem.getWaimaiPrice());
                map.put("source",hyOrder.getSource());
				list.add(map);
			}
