...
@Controller
@RequestMapping("/admin/storeLineOrderScg/")
public class StoreLineOrderScgController {
...
//				}
	...
	@ResponseBody
	public Json canscel(Long id, HttpSession session) {
		/**
		 * 获取当前用户
		 */
		String username = (String) session.getAttribute(CommonAttributes.Principal);
	...
	public Json storeCancelGroupApply(@RequestBody HyOrderApplication application,Boolean isOnlyBaoxian, HttpSession session) {
		...
		try {
			...
			if(isOnlyBaoxian){
				...
				taskService.addComment(task.getId(), provider.getUsername(),
						("审核通过") + ":" + 1);
...
//					HySupplierContract contract = new HySupplierContract();
//					List<Filter> filters = new ArrayList<>();
//		            filters.add(Filter.eq("liable", hyOrder.getSupplier()));
//		            List<HySupplierContract> contracts = hySupplierContractService.findList(null,filters,null);
//		            for(HySupplierContract con : contracts){
//		            	if(con.getContractStatus().equals(ContractStatus.zhengchang)){
//		            		contract = con;
...
//		            	throw new Exception("没有正常状态的合同");
					...
					//add start by cqx 6.25
					if(hyOrder.getRefundstatus()==3) {//如果是部分退团
						String contact=hyOrder.getContact();//拿到该订单的联系人
						List<HyOrderApplicationItem> HyOrderApplicationItemsList= application.getHyOrderApplicationItems();
						List<HyOrderCustomer> hyOrderCustomerListTuiTuan=new ArrayList<>();
						List<HyOrderItem> hyOrderItemList=hyOrder.getOrderItems();
						List<HyOrderCustomer> hyOrderCustomerListAll=new ArrayList<>();
						for(HyOrderApplicationItem temp:HyOrderApplicationItemsList) {
							if(hyOrderItemService.find(temp.getItemId()).getHyOrderCustomers()!=null) {
								hyOrderCustomerListTuiTuan.addAll(hyOrderItemService.find(temp.getItemId()).getHyOrderCustomers());//拿到申请退团的人的list
							}
							else {
								throw new Exception("部分退团审核出错");
							}
						}
						Long minCustomerId=null;
						for(HyOrderItem flag:hyOrderItemList) {
							hyOrderCustomerListAll.addAll(flag.getHyOrderCustomers());//拿到该orderid的所有customer
						}
//						if(hyOrderCustomerListAll.removeAll(hyOrderCustomerListTuiTuan)) {//求两个差集
//							if(!hyOrderCustomerListAll.isEmpty()) {
						minCustomerId=hyOrderCustomerListAll.get(0).getId();
						for(HyOrderCustomer tmp:hyOrderCustomerListAll) {
							if(minCustomerId>tmp.getId()) {
								minCustomerId=tmp.getId();//拿到最小的customerId
							}
						}
//							}
//							else {
//								throw new Exception("部分退团导致游客有空");
//							}
					
						
						for(HyOrderCustomer temp:hyOrderCustomerListTuiTuan) {
							if(contact.equals(temp.getName())) {//部分退团的人含有联系人
									hyOrder.setContact(hyOrderCustomerService.find(minCustomerId).getName());
									hyOrder.setContactIdNumber(hyOrderCustomerService.find(minCustomerId).getCertificate());
									hyOrder.setPhone(hyOrderCustomerService.find(minCustomerId).getPhone());
					...
					refundPayServicerModifyService.tuituan(hyOrder, application, contract, username, store);
...
//
//
//					if(hyOrder.getSource()==0){	//如果来源是门店
...
//							// add by wj
//							HyOrderItem orderItem = hyOrderItemService.find(item.getId());
//							if(orderItem!= null && orderItem.getType() == 1)
