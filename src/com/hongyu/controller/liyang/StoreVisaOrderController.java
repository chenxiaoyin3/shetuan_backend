 ...
 * 门店签证订单管理中心
 * @author liyang
 * @version 2018年11月8日 上午10:17:41
 */
@Controller
@RequestMapping("/admin/storeVisaOrder/")
public class StoreVisaOrderController {
	 ...
	 * @param source 订单来源
	 * @param continentId 大洲
	 * @param leftCreateTime 创建时间
	 * @param rightCreateTime 创建时间
	 * @param pageable
	 * @param session
	 * @param request
	...
	public Json list(String name,String orderNumber,Integer payStatus,
		...
		try {
			...
			if(source != null && source >= 0 && source <= 1){
				...
				filters.add(Filter.ne("source", 2));
			}
			if(orderNumber != null){
...
//	}
	...
	public Json detail(Long id){
		...
		try {
			...
			/*因为现在前端显示的jiesuanMoney1还是jiusuanMoney中的额一个，但是现在如果是官网选门店的话，要显示官网外卖价
			 * 为了不让前端修改，就判断当订单来源是source =2官网选门店的时候，显示外卖价
			 * */
			if(tmp.getSource()==0){
	 ...
	 * @return
	 */
	@RequestMapping("adjustMoney")
	@ResponseBody
	public Json adjustMoney(Long id,BigDecimal adjustMoney){
		Json json = new Json();
		try {
			HyOrder order = hyOrderService.find(id);
			if (order == null) {
	 ...
	 * @return
	 */
	@RequestMapping("addReceiptRefund")
	@ResponseBody
	...
	public Json cancel(Long id, HttpSession session) {
		...
		try {
			...
			hyOrderService.cancelVisaOrder(id);
	...
	@ResponseBody
	public Json listForProvider(String name,String orderNumber,Integer payStatus,Integer confirmStatus,Integer refundStatus,
			Integer continentId,Pageable pageable,HttpSession session,HttpServletRequest request){
		Json json = new Json();
		try {
			/**
