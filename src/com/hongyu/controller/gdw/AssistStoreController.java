...
@Controller
@RequestMapping("/admin/assistStore/")
public class AssistStoreController {
	...
	public Json add(Assist assist,HttpSession session){
		...
		try {
			...
			assist.setAssistType(Assist.storeType);
	...
	public Json list(Pageable pageable,Assist assist,HttpSession session,HttpServletRequest request){
		...
		try {
			...
			filters.add(filter);
			filters.add(Filter.eq("assistType", Assist.storeType));
