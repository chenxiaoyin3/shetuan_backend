...
@Controller
@RequestMapping("/admin/assistLegal/")
public class AssistLegalController {
	...
	public Json add(Assist assist,HttpSession session){
		...
		try {
			...
			assist.setAssistType(Assist.legalType);
	...
	public Json list(Pageable pageable,Assist assist,HttpSession session,HttpServletRequest request){
		...
		try {
			...
			filters.add(filter);
			filters.add(Filter.eq("assistType", Assist.legalType));
