...
@Controller
@RequestMapping("/admin/storeLineOrderProvider/")
public class StoreLineOrderProviderController {
	...
	public Json list(Pageable pageable, @DateTimeFormat(pattern="yyyy-MM-dd") Date startTime, 
			@DateTimeFormat(pattern="yyyy-MM-dd") Date endTime,@DateTimeFormat(pattern="yyyy-MM-dd") Date startDate,
...
//				}
	...
	@ResponseBody
	public Json gysList(Pageable pageable, @DateTimeFormat(pattern="yyyy-MM-dd") Date startDate,
						@DateTimeFormat(pattern="yyyy-MM-dd") Date endDate,HyOrder hyOrder, HttpSession session,HttpServletRequest request) {
		Json json = new Json();
		try {
			/**
			 * 获取当前用户
		...
		try {
			...
			for (HyOrderItem item : order.getOrderItems()) {
				...
				myOrderItem.setJiesuanRefund(item.getJiesuanPrice().subtract(group.getFanliMoney().multiply(
						BigDecimal.valueOf(item.getNumber()-item.getNumberOfReturn()))).multiply(lineRefundPercentage));
		...
		try {
			...
			if(order.getIfjiesuan()==false){	//如果没有结算
				if(order.getKoudianMethod().equals(Constants.DeductLine.tuanke.ordinal())){
					...
					BigDecimal orderMoney = order.getJiesuanMoney1().add(order.getTip())
