...
import com.hongyu.controller.lbc.InsuranceOrderCenterController.MyOrderItem;
...
@Controller
@RequestMapping("admin/ticket_subscribe_ordercenter/")
public class HyTicketSubscribeOrderCenterController {
	...
	public Json pageView(Pageable pageable,Integer payStatus,Integer refundStatus,Integer star,
		...
		try {
			...
			String[] attrs = new String[]{
					"id","status","orderNumber","sceneName","star","startDate","createTime","operator","source"
			};
			
			//订单type 2 认购门票
			
			StringBuilder totalSb = new StringBuilder("select count(*)");
			StringBuilder pageSb = new StringBuilder("select o1.id,o1.status,o1.order_number,h1.scene_name,"
			...
			StringBuilder sb = new StringBuilder(" from hy_order o1,hy_order_item i1,hy_ticket_subscribe h1"
					+ " where o1.type=2 and o1.id=i1.order_id and i1.product_id=h1.id and i1.price_type=0 and h1.sale_status=2 and h1.status=1");
...
//				sb.append(" and c1.name like '%" + customerName + "%'");
//			}
...
//			}
//			if(sceneName!=null){
//				sb.append(" and scene_name like '%"+sceneName+"%'");
//			}
//			if(orderNumber!=null){
...
//	}
	...
	@ResponseBody
	public Json storePayDetail(Long id,HttpSession session)
	{
		Json json=new Json();
		try {
			HyOrder hyOrder = hyOrderService.find(id);
...
//	}
	...
	public Json storeCancelGroupApply(@RequestBody HyOrderApplication application, HttpSession session) {
		...
		try {
			...
			/**找供应商需要注意*/
			//找出供应商
			HyAdmin provider = hyTicketSubscribe.getCreator();
			// 指定审核供应商
