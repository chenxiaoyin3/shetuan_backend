...
@Controller
@RequestMapping("admin/innerSupplier/profit")
public class InnerSupplierProfit {
...
//	 * 根据登录账号判断是经理还是员工接口
//	*/
//	@RequestMapping("isManager/judge")
//	@ResponseBody
//	public Json judgeManager(HttpSession session)
//	{
//		Json json=new Json();
//		try {
...
//	}
	 ...
	 * 建团计调登录查询接口
	*/
	@RequestMapping("empoyeeList/view")
	@ResponseBody
	public Json employeeListview(String usename,@DateTimeFormat(pattern="yyyy-MM-dd") Date startTime,@DateTimeFormat(pattern="yyyy-MM-dd") Date endTime,HttpSession session)
	{
		Json json=new Json();
		try {
			...
			filters.add(Filter.eq("creator", hyAdmin));  //建团计调筛选
			filters.add(Filter.eq("isInner", true)); //筛选内部供应商团
			//按发团日期 的时间筛选
			if(startTime!=null) {
				filters.add(new Filter("startDay", Operator.ge, DateUtil.getStartOfDay(startTime)));
			}
			if(endTime!=null) {
				filters.add(new Filter("startDay", Operator.le, DateUtil.getEndOfDay(endTime)));
			}
			...
			for(HyGroup hyGroup:hyGroups) {	
				...
				if(hyRegulates.size()>0) {
					...
					if(hyRegulate.getStatus()==2) { //计调报账申请通过
						Map<String, Object> map = new HashMap<String, Object>();
						map.put("productId", hyGroup.getGroupLinePn());  //产品ID
						map.put("startDay", hyGroup.getStartDay()); //发团日期
						...
						if(regulategroupAccounts.isEmpty()) {
							map.put("allIncome", null);
							map.put("allExpense", null);
							map.put("profit", null);
						}
						else {
							/**只有收入和支出都不为零,才返回*/
							if(regulategroupAccounts.get(0).getAllIncome()!=BigDecimal.ZERO && 
									regulategroupAccounts.get(0).getAllExpense()!=BigDecimal.ZERO) {
								map.put("allIncome", regulategroupAccounts.get(0).getAllIncome()); //总收入
								map.put("allExpense", regulategroupAccounts.get(0).getAllExpense()); //总支出
								map.put("profit", regulategroupAccounts.get(0).getProfit()); //利润
								list.add(map);
							}
						}		
					}	
			...
			Collections.sort(list, new Comparator<Map<String, Object>>() {
				...
				public int compare(Map<String, Object> o1, Map<String, Object> o2) {
					Date date1 = (Date) o1.get("startDay");
					Date date2 = (Date) o2.get("startDay");
...
//			List<HyRegulateitemElement> hyRegulateitemElements=new ArrayList<>();
	...
	*/
	...
	public class InnerSupplierProfitEmployee{
	 ...
	 * 建团计调登录导出excel接口
	*/
	@RequestMapping("empoyeeList/excel")
	public String employeeExcel(@DateTimeFormat(pattern="yyyy-MM-dd") Date startTime,@DateTimeFormat(pattern="yyyy-MM-dd") Date endTime,
			HttpSession session,HttpServletRequest request, HttpServletResponse response)
	{
		try {
			...
			List<HyGroup> hyGroups=hyGroupService.findList(null,filters,null);
			filters.clear();
		    List<InnerSupplierProfitEmployee> results=new ArrayList<>();
			for(HyGroup hyGroup:hyGroups) {	
				Long groupId=hyGroup.getId();
				filters.add(Filter.eq("hyGroup", groupId)); //根据团找该团的计调
				List<HyRegulate> hyRegulates=hyRegulateService.findList(null,filters,null);
				...
				if(hyRegulates.size()>0) {
					...
					HyRegulate hyRegulate=hyRegulates.get(0); //得到计调报账
					if(hyRegulate.getStatus()==2) { //计调报账申请通过
						InnerSupplierProfitEmployee map=new InnerSupplierProfitEmployee();
						map.setProductID(hyGroup.getGroupLinePn());  //产品ID
						SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
						map.setStartDay(sdf.format(hyGroup.getStartDay())); //发团日期
						filters.add(Filter.eq("groupId", hyGroup.getId())); 
						//在单团核算表中找出该团的总收入,总支出和利润
						List<RegulategroupAccount> regulategroupAccounts=regulategroupAccountService.findList(null,filters,null);
						filters.clear();
						if(regulategroupAccounts.isEmpty()) {
							map.setAllIncome(null);
							map.setAllExpense(null);
							map.setProfit(null);
						}
						else {
							if(regulategroupAccounts.get(0).getAllIncome()!=BigDecimal.ZERO && 
									regulategroupAccounts.get(0).getAllExpense()!=BigDecimal.ZERO) {
								map.setAllIncome(regulategroupAccounts.get(0).getAllIncome().toString()); //总收入
								map.setAllExpense(regulategroupAccounts.get(0).getAllExpense().toString()); //总支出
								map.setProfit(regulategroupAccounts.get(0).getProfit().toString()); //利润
								results.add(map);
							}			
						}	
			...
			//按发团日期倒序排列
			Collections.sort(results, new Comparator<InnerSupplierProfitEmployee>() {
				@Override
				public int compare(InnerSupplierProfitEmployee o1,InnerSupplierProfitEmployee o2) {
					String date1 = o1.getStartDay();
					String date2 = o2.getStartDay();
					return date1.compareTo(date2); 
				}
			});
			...
			String configFile = "innerSupplierProfitEmpoyee.xml"; // 配置文件
