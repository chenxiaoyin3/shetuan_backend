...
@Component("njtInsureProcessor")
public class NJtInsuranceProcess implements Processor{
	...
	private List<Long> getOrderList() throws Exception{
		...
		filters.add(Filter.ge("fatuandate", sdf.parse(DateUtil.getSimpleStartOfDayDate(nextDay))));
		filters.add(Filter.le("fatuandate", sdf.parse(DateUtil.getSimpleStartOfDayDate(nextDay))));
		
		List<HyOrder> hyOrders = hyOrderService.findList(null,filters,null);
		System.out.println("size = "+hyOrders.size());
		if(hyOrders != null){
			for(HyOrder hyOrder:hyOrders){
				insuranceOrderService.generate(hyOrder.getId());
				//System.out.println("hyorderid = "+hyOrder.getId());
				hyOrderIds.add(hyOrder.getId());
			}
		}
		return hyOrderIds;
	}
